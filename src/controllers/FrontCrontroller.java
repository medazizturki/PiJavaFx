...
public class FrontCrontroller implements Initializable {
    ...
    private void ajouter_reclamation(ActionEvent event) throws Exception {
...

        {
            if (txtnom_reclamation.getText().equals("")) {
                ...
                if(i>3)
                {
                    e.banuser(user.getId());
                                    AlertDialog.showNotification("Error !", "user is banned", AlertDialog.image_cross);
    ...
    private void arabe(ActionEvent event) {
            ...
            try {
             FXMLLoader loader = new FXMLLoader(getClass().getResource("/gui/fronteya_1.fxml"));
             Front1Crontroller c = new Front1Crontroller();
             Parent root = loader.load();
             Scene scene = new Scene(root);
             Stage stage = (Stage) ((Node) event.getSource()).getScene().getWindow();
             stage.setScene(scene);
              String path = "arabic.mp3";
